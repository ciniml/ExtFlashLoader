...
std::uint8_t boot_bin[] = {
  0x00, 0x00, 0x03, 0x20, 0x09, 0x00, 0x00, 0x04, 0x80, 0xb5, 0x00, 0xaf,
  0x86, 0xb0, 0x00, 0xf0, 0x35, 0xf8, 0x05, 0x90, 0x05, 0x98, 0x01, 0x21,
  0xca, 0x03, 0x82, 0x60, 0x02, 0x91, 0x00, 0xf0, 0x35, 0xf8, 0x04, 0x90,
  0x04, 0x98, 0x02, 0x99, 0xca, 0x02, 0x82, 0x60, 0x04, 0x98, 0x82, 0x61,
  0xff, 0xe7, 0x05, 0x98, 0x01, 0x21, 0xca, 0x03, 0xc2, 0x61, 0x04, 0x98,
  0xc9, 0x02, 0xc1, 0x61, 0x00, 0x20, 0x03, 0x90, 0xff, 0xe7, 0x03, 0x98,
  0xc0, 0x08, 0x7c, 0x28, 0x06, 0xd8, 0xff, 0xe7, 0x03, 0x98, 0x41, 0x1c,
  0x81, 0x42, 0x01, 0x91, 0x01, 0xd3, 0x04, 0xe0, 0xe9, 0xe7, 0x03, 0x48,
  0x00, 0x21, 0x00, 0xf0, 0x05, 0xf8, 0x01, 0x98, 0x03, 0x90, 0xec, 0xe7,
  0x04, 0x01, 0x00, 0x04, 0x81, 0xb0, 0x00, 0x91, 0xff, 0xe7, 0xfe, 0xde,
...
void setup() {
    ...
    for(std::uint_fast16_t i = 0; i < 256; i++) {
        pageData[i] = static_cast<std::uint8_t>(i);
    }
    // qspiFlash.writeEnable();
    // qspiFlash.programPage(0, pageData, sizeof(pageData));
    // qspiFlash.waitProgram(0);

    Serial.println("Verifying page...");
    std::uint8_t verifyBuffer[sizeof(pageData)];
    qspiFlash.readData(0, verifyBuffer, sizeof(verifyBuffer));
    for(std::uint_fast16_t i = 0; i < 256; i++) {
        if( pageData[i] != verifyBuffer[i]) {
            Serial.printf("%04x: %02x != %02x\r\n", i, verifyBuffer[i], pageData[i]);
        }
    }
    Serial.println("Done!");
    ...
    for(std::uint_fast16_t i = 0; i < 256; i++) {
        if( pageData[i] != mapped[i]) {
            Serial.printf("%04x: %02x != %02x\r\n", i, mapped[i], pageData[i]);
        }
    }
    Serial.println("Done!");
